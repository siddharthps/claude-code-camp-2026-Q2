...
class Tui(App):
    ...
    def _render_progress(self):
        ...
        if self._live["active"]:
            ...
            used = self._fmt_tokens(context.current_tokens)
            max_ = self._fmt_tokens(context.context_window)
            turns = self._turn_count
            widget.update(
                f"[{color}]  [ready]   ctx {used} / {max_} ({pct}%)   {turns} turns[/{color}]"
            )
    ...
    def _render_status(self):
        ...
        bar = (
            f" boukensha v{version} · {model}  ·  ctx {used}/{max_} ({pct}%)"
            f"{indicator}·  {tools} tools  ·  {clock} "
        )
        width = self.size.width or 80
        self.query_one("#status", Static).update(bar.ljust(width))

    @classmethod
    def _ctx_color(cls, pct):
